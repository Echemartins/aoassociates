...
export function ProjectHeroGallery({ images }: { images: Img[] }) {
  ...
  // ESC close + body scroll lock + arrow navigation inside modal
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      if (e.key === "Escape") setOpen(false)
      if (!open) return
  ...
  return (
    <div className="mt-1">
      ...
      <div className="mx-auto w-full">
        <div className="relative overflow-hidden border border-[rgb(var(--border))] bg-[rgb(var(--card))]">
          <div className="relative h-[78vh] min-h-[280px] max-h-[880px] w-full">
            ...
            <button
              ...
              aria-label="Previous image"
              className={[
                "absolute left-3 top-1/2 -translate-y-1/2 z-20 inline-flex h-10 w-10 items-center justify-center rounded-full border",
                "border-[rgb(var(--border))] bg-[rgba(255,255,255,0.80)] backdrop-blur",
                "transition-opacity hover:opacity-95",
                !canPrev ? "opacity-35 cursor-not-allowed" : "",
              ].join(" ")}
            ...
            <button
              ...
              aria-label="Next image"
              className={[
                "absolute right-3 top-1/2 -translate-y-1/2 z-20 inline-flex h-10 w-10 items-center justify-center rounded-full border",
                "border-[rgb(var(--border))] bg-[rgba(255,255,255,0.80)] backdrop-blur",
                "transition-opacity hover:opacity-95",
                !canNext ? "opacity-35 cursor-not-allowed" : "",
              ].join(" ")}
            ...
            {/* INDEX CHIP */}
            <div className="absolute bottom-3 left-3 z-20 rounded-full border border-[rgb(var(--border))] bg-[rgba(255,255,255,0.80)] px-3 py-1 text-xs text-[rgb(var(--muted))] backdrop-blur">
              {index + 1} / {list.length}
            </div>
          </div>
        </div>
        ...
        <p className="mt-2 text-l text-[rgb(var(--muted))]">
          Click the main image to view details. Use arrows to browse.
        </p>
      </div>
      ...
      {open ? (
        <div className="fixed inset-0 z-50" role="dialog" aria-modal="true">
          ...
          {/* Modal */}
          <div className="absolute inset-0 flex items-center justify-center">
            <div className="relative h-[92vh] w-[92vw] max-w-7xl overflow-y-auto md:overflow-hidden rounded border border-[rgb(var(--border))] bg-[rgb(var(--bg))] shadow-xl">
              {/* Close (absolute) */}
              <button
                type="button"
                onClick={closeModal}
              ...
              {/* 3-column layout: Logo | Details | Image */}
              <div className="grid h-full w-full md:grid-cols-12">
                {/* Logo */}
                <div className="md:col-span-2 border-b border-[rgb(var(--border))] md:border-b-0 md:border-r bg-[rgb(var(--card))]">
                  <div className="h-full w-full flex pt-10 justify-center">
                    ...
                    <div className="px-3 py-3 grid gap-3 text-sm">
                      {modalImg.caption ? (
                        <div>
                          ...
                          <div className="mt-1 text-2xl text-[rgb(var(--fg))]">
                            {modalImg.caption}
                          </div>
                        </div>
                      ) : null}
                      ...
                      {!modalImg.caption && !modalImg.credit && !modalImg.notes ? (
                        <div className="text-[rgb(var(--muted))]">
                          No additional details for this image.
                        </div>
                      ) : null}
                    </div>
                  </div>
                ...
                {/* Image */}
                <div className="md:col-span-7 bg-[rgb(var(--card))] min-h-[55vh] md:min-h-0">
                  <div className="relative h-[55vh] md:h-full w-full">
                    <div className="absolute inset-0">
                      <MediaFill
                        src={modalImg.url}
                        alt={modalImg.alt || "Project image"}
