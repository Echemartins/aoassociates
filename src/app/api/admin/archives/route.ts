...
async function makeUniqueArchiveSlug(base: string) {
...
export async function POST(req: NextRequest) {
  ...
  const slug = await makeUniqueArchiveSlug(baseSlug)
  ...
  const created = await prisma.archiveProject.create({
    data: {
      slug,
      title: data.title,
  ...
  return NextResponse.json({ archive: created })
}
