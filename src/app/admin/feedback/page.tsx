...
export default function AdminFeedbackPage() {
  ...
  async function load() {
    ...
    if (!res.ok) setError(json?.error || "Failed to load feedback.")
  ...
  async function remove(id: string) {
    const ok = confirm("Delete this feedback permanently?")
    if (!ok) return
    setBusyId(id)
    setError(null)
    ...
    <div className="rounded-2xl border border-[rgb(var(--border))] bg-white p-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-[rgb(var(--muted))]">
            Site feedback submitted by visitors (shows up here for admin follow-up).
          </p>
        </div>
        <button
          onClick={load}
      ...
      {loading ? (
        ...
        <div className="mt-6 grid gap-4">
          ...
          {!items.length ? (
            <div className="text-sm text-[rgb(var(--muted))]">No feedback yet.</div>
          ) : null}
        </div>
      )}
    </div>
